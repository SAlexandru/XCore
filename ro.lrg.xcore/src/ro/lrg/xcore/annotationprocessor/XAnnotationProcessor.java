...
import ro.lrg.xcore.preferencepage.XCorexPropertyPage.XCorePropertyStore;
...
public class XAnnotationProcessor extends AbstractProcessor {
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		...
		processRelationBuilder(roundEnv);
	...
	private void processRelationBuilder(RoundEnvironment env) {
		for (Element elem: env.getElementsAnnotatedWith(RelationBuilder.class)) {
			if (ElementKind.CLASS != elem.getKind()) {
				printError (elem, "@RelationBuilder must annotate classes!");
			}
			else {
				TypeElement tElem = (TypeElement)elem;
				if (tElem.getSimpleName().equals(tElem.getQualifiedName())) {
				...
				try {
					XRelationBuilderGenerator gb = new XRelationBuilderGenerator ((TypeElement)elem);
					gb.setElementUtils(processingEnv.getElementUtils());
					generator_.createEntity(gb.getEntityType()).addRelationBuilder(gb);
					...
					printError (elem, "For IRelationBuilder specify concrete type, not ? [exteds/super T]");
				}
			}
		}
	}
